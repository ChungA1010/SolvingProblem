...
**특징·연마 구간·이력 순차 개선 v4 완료 (2026-09-20):** [전체 결과와 단계별 비교](runs/phm_cmp_staged_v4/README.md). 동일한 분할에서 125/73/12/33특징, 다섯 구간 집계, 다섯 이력 방식과 조건별 평균/직전값 대체를 내부 검증으로 순차 선택했습니다. 기존 완료 이력 `robust v2` 대비 그룹 RMSE는 **4.6658 → 4.3594 (6.57% 감소)**였지만, 시간순은 **3.4309 → 4.1632 (21.34% 증가)**, 기존 Test는 **2.9915 → 3.5088 (17.29% 증가)**였습니다. 새 후보는 채택하지 않고 기존 연구 모델과 API/Unity 모델을 유지합니다. 12변수안은 계산식을 명시한 검증용 정의이며 Claude 자료의 성능 재현을 주장하지 않습니다. [고정 계획](docs/staged-improvement-v4.md), [저장 모델·예측 봉인](runs/phm_cmp_staged_v4/training_complete.json), [재검증](runs/phm_cmp_staged_v4/verification.json).

**논문 모델별 최신 비교 (2026-09-20):** [발표 성능·현재 재현·staged v4 전체 비교표](runs/phm_cmp_paper_comparison_v4/README.md). P1은 Stage A의 다섯 모델 모두 발표 RMSE보다 높고, Stage B는 다섯 중 네 모델이 낮습니다. P2 통합 MSE는 **7.07 대 7.0743**으로 가깝지만 LR 단독은 큰 격차가 남습니다. P3의 보정 RF는 **7.4 대 8.6851**, GA 보완 후 **9.0837**입니다. 미구현·별도 Test 미평가 모델도 명시했으며, 평가 조건 차이로 완전 재현이나 우월성을 주장하지 않습니다.

## 세 논문 원문과 구현 성능의 최종 비교 (2026-09-19)

**누락 절차 보완 v3 완료:** [85개 특징 선택·P2 가정 비교·실제 GA 실행 결과](runs/phm_cmp_completion_v3/README.md). P1은 1,440회 기본 모델 비교와 fold 안의 특징 선택을 구현했고 ELM Test RMSE가 A **8.9624 → 7.8971**, B **4.5035 → 4.2687**로 낮아졌습니다. P2의 일반 LR 대조군은 통합 MSE **7.0743**으로 기존과 동일했습니다. P3는 45/12개 재구성 후보의 GA를 실행했지만 같은 1,977행에서 CPP 보정 MSE **8.6851 → 9.0837**로 악화됐습니다. 원문에 전체 목록이 없는 rough 47개를 정확히 복원했다고 주장하지 않습니다. 극단값 네 개의 포함 여부도 별도로 비교했습니다. 32개 모델 파일·944개 지표를 재검증했고 이전 산출물 1,171개를 보존했습니다. 전체 테스트 148개 통과. 기존 API/Unity 모델은 유지합니다.

**후속 챔버 특징 제외 비교 완료:** [학습·검증 결과](runs/phm_cmp_secondary_ablation_v1/README.md). 125개 입력에서 후속 챔버 52개만 제외하고 같은 절차로 다시 학습했습니다. 기존 Test MSE는 **7.0743 → 7.0317**, 그룹 검증은 **36.4062 → 35.6296**으로 낮아졌지만, 시간순은 **32.4365 → 36.2000**, Validation은 **6.8277 → 6.9603**으로 악화됐습니다. 시간순의 세 조건 모두 악화돼 일괄 제외 후보를 채택하지 않고 기존 모델을 유지합니다. 21개 적합·내부 420개 분할·288개 지표를 재검증하고 이전 산출물 950개를 보존했습니다. 전체 테스트 142개를 통과했습니다.

**Train 챔버 신호 확인 완료:** [원시 로그 검사 결과·실제 신호 그림](runs/phm_cmp_chamber_audit_v1/README.md). 1,981개 wafer-stage 중 738개에서 후속 챔버와 다른 웨이퍼 주 연마의 19개 센서가 같은 시각에 연속 일치했습니다. 검사 가능한 후속 챔버 행의 37.92%이며, 이전 웨이퍼로부터의 지연 복사라는 최초 가설은 이번 기준에서 확인하지 못했습니다. 강한 일치 쌍은 모두 기존의 같은 wafer/file 그룹 안에 있습니다. 새 학습 없이 신호 기록을 점검했고 전체 테스트 135개를 통과했습니다.

**논문 재검토와 다음 실험 후보:** [원문 근거·현재 구현·검증 계획](docs/paper-guided-next-experiments.md)을 정리했습니다. 우선순위는 후속 챔버 신호의 출처 확인, 재시작·소모품 상태에 맞는 이력 선택, P1의 전체 특징 선택 과정 보완입니다. 이번 추가 문서는 연구 검토이며 새 학습 결과나 성능 개선 확인을 뜻하지 않습니다.

**후속 P2 오차 진단:** [LR 영향 분석·제한된 비교](runs/phm_cmp_lr_diagnosis_v1/README.md)를 완료했습니다. 선형 모델 단독 Test MSE는 Ridge 교체로 **351.1079 → 17.6054**가 됐지만, 통합 모델은 기존 **7.0743**, LR 제거 **7.0778**, Ridge 교체 **7.1687**로 추가 개선에 실패했습니다. 기존 모델은 유지합니다. 큰 LR 오차가 발생한 Cond1의 기존 LR 가중치는 이미 약 **5.37e-7**이었고, 상위 통합 오류 10건은 LR을 제외한 네 구성 모델이 모두 같은 방향으로 틀렸습니다. 이 표본들은 제곱오차의 **31.4%**를 차지합니다. 기존 산출물 844개를 보존하고 전체 테스트 125개를 통과했습니다.
