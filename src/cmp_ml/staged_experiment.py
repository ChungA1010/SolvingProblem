...
def context():
    ...
    expected=p['code_sha256']
    amendment=RUN/'amendments/01_reference_intervals.json'
    if amendment.exists():
        a=read_json(amendment)
        assert a['original_code_sha256']==expected
        check_hash(RUN/'protocol.json',a['original_protocol_sha256'])
        expected=a['updated_code_sha256']
    assert code_hashes()==expected,'Frozen code changed without a recorded amendment'
    ...
    for evaluation,b in p.groupby('evaluation'):
        for base in ('full125_rf','full125_bag'):
            ...
            for candidate in ('S1_features','S2_phase','S3_history','S4_final'):
                ...
                if q.group_id.isna().any():
                    # Official reference caches have no connected-group IDs. Do not
                    # invent IID rows or silently bootstrap an empty group table.
                    old=float(np.sqrt(np.square(ref.prediction-ref.truth).mean()))
                    new=float(np.sqrt(np.square(q.prediction-q.truth).mean()))
                    rows.append({'evaluation':evaluation,'baseline':base,'candidate':candidate,'groups':0,
                                 'baseline_rmse':old,'candidate_rmse':new,'rmse_improvement_pct':100*(1-new/old),
                                 'bootstrap_low_pct':None,'bootstrap_high_pct':None,
                                 'interval_scope':'unavailable: reference connected-group metadata absent; no IID-row replacement'})
                    continue
